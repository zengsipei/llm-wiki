...
created: 2026-06-01T08:52:57.862Z
updated: 2026-06-01T10:37:22.470Z
---
...
## 1. mini-swe-agent：100行代码的极简 Coding Agent（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）

| 属性 | 详情 |
|------|------|
| GitHub | https://github.com/SWE-agent/mini-swe-agent |（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）
| Stars | ~4,754 |
| 语言 | Python |
...
```bash
pip install uv && uvx mini-swe-agent        # 快速体验，无需安装（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）
pip install mini-swe-agent && mini           # 完整安装（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）
```
...
| 维度 | mini-swe-agent | smolagents | Mini-Agent |（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）
|------|----------------|------------|------------|
| 定位 | 最小骨架 | 通用框架 | 工程模板 |
...
按帖子的递进顺序阅读：先看 mini-swe-agent 的 100 行代码理解 Agent 最小循环，再看 smolagents 理解"代码即动作"的 ReAct 模式和工具系统设计，最后看 Mini-Agent 学习持久化记忆、上下文管理等工程化实践。三个项目读完，对 Agent 的理解会从"知道概念"变成"能自己搭一个"。（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）

## 与 Claude Code 的关联

这三个项目的设计理念与 Claude Code 的 Operator 模式有直接的对比价值：Claude Code 同样采用 Bash 作为核心执行环境、线性历史记录、极简工具抽象，但在此基础上增加了 IDE 集成、权限控制、多文件编辑等工程能力。mini-swe-agent 可以视为 Claude Code Coding Agent 部分的"学术极简版"。（详见 [[mini-swe-agent — 100行代码的极简 Coding Agent]]）