...
// Cross-reference mapping: page title -> array of related page titles
const crossRefs = {
  "Agent 架构设计模式": [
...
async function main() {
  ...
  for (const page of allPages) {
    titleToId.set(page.title.trim(), page.id);
  }
  console.log(`Title->ID map built with ${titleToId.size} entries`);
  ...
  for (const [title, refTitles] of Object.entries(crossRefs)) {
    ...
    // Resolve ref titles to IDs
    const refIds = [];
    for (const refTitle of refTitles) {
      const refId = titleToId.get(refTitle);
      if (refId) {
        refIds.push(refId);
      } else {
        console.warn(`  ⚠️  Ref not found for "${title}" -> "${refTitle}"`);
      }
    }
    ...
    if (refIds.length > 0) {
      ...
      updated++;
      console.log(`✅ ${title}: ${refIds.length} cross-refs`);
    }
  }
  
  console.log(`\n--- Summary ---`);
  console.log(`Updated: ${updated} pages`);
  console.log(`Not found: ${notFound.length} pages`);
  if (notFound.length > 0) {
    console.log(`Missing pages: ${notFound.map(t => `"${t}"`).join(', ')}`);
  }
  
  // Verify: count pages with non-empty backlinks
  const withRefs = await db.wikiPage.findMany({
    where: { backlinks: { not: '[]' } },
    select: { title: true },
  });
  console.log(`Pages with cross-refs: ${withRefs.length}/${allPages.length}`);
