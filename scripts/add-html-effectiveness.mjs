// Seed script: Add "The Unreasonable Effectiveness of HTML" to Wiki
// Run: node scripts/add-html-effectiveness.mjs
import { PrismaClient } from '@prisma/client';
const prisma = new PrismaClient();

const CONTENT = `# HTML 的不合理有效性
...
async function main() {
  const page = await prisma.wikiPage.create({
    data: {
      ...
      content: CONTENT,
      pageType: 'concept',
      tags: JSON.stringify(['AI', 'HTML', 'Agent', 'Output-Format', 'Design', 'Prototyping', 'Artifact']),
      backlinks: '[]',
    },
  });
